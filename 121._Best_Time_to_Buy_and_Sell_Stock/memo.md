...
- `itertools.accumulate` 使って処理を関心で分離してみる
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        for selling, buying in zip(prices, acc_min_prices):
            max_prof = max(max_prof, selling - buying)
        return max_prof
...
- `__find_center_crossing_max_profit` で`itertools.islice`を使う例も試したが，`itertools.islice` は`start, stop`が非負整数かつ `step` は正整数を要求するので，使えなかった．
  - スライスで書いても，`arr[center - 1 : -1 : -1] = []`となってしまい，今回ほしいものである`[arr[center - 1], ..., arr[1], arr[0]]` は取れない．
    - こうなる理由は要調査．理解では以下の挙動をするから；
      1. $N=len(arr)$としてインデックス `start`,`stop`の評価 ($[-N, N]$にクリップしてから負の数は$+N$して非負にする)
      2. `start`と`step`が等しい，あるいは，$\text{stop} - \text{start}$と`stop`が異符号であるときは空配列を返す．
      3. そうでなければ指定されたインデックスの要素を取り出して配列にして返す．
  - `itertools.accumulate` を使うとLeetCodeのTLを超えたのと，別に元のコードで十分読みやすいので，必要性が大きくない．
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        return max(0, self.__max_profit(price_changes, 0, len(price_changes)))

    def __max_profit(self, price_changes: List[int], begin: int, end: int) -> int:
        if begin >= end:
            return 0
        if end - begin == 1:
            return price_changes[begin]
        center = (begin + end) // 2
    ...
    def __find_center_crossing_max_profit(
        ...
        if not begin < center < end:
            return float("-inf")
        left_sum = price_changes[center - 1]
        left_max_sum = left_sum
        for i in range(center - 2, begin - 1, -1):
            left_sum += price_changes[i]
            left_max_sum = max(left_max_sum, left_sum)

        right_sum = price_changes[center]
        right_max_sum = right_sum
        for i in range(center + 1, end):
            right_sum += price_changes[i]
            right_max_sum = max(right_max_sum, right_sum)
...
- 好みの範疇だが，階差数列`price_changes`の計算に`itertools.islice`を使うと， for each 形式でかけてインデックスの細々したところがなくなってわかりよいかな？
  - スライス `prices[1:]` と(直接)対応することと，記述の簡潔さから，`itertools.islice(prices, 1, None)` のほうが `itertools.islice(prices, 1, len(prices))` より好み．
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        price_changes = (
            tomorrow - today
            for today, tomorrow in zip(prices, itertools.islice(prices, 1, None))
        )
        for change in price_changes:
            today_max_profit = max(today_max_profit + change, change)
            max_prof = max(max_prof, today_max_profit)
        return max_prof
